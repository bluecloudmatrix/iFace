 ...
 */
//img是一个rows行cols列的二维数组，存有每个像素点的灰度值
function haarDetect(img, cascade, scale_factor) {
    ...
    for(factor = 1; factor*cascade.orig_window_size.width < cols - 10  && factor*cascade.orig_window_size.height < rows - 10; factor *= scale_factor) {
...
function calc_sum(rect, ix, iy, sum, rows) {
    ...
    p0 = sum[iy + rect.y][ix + rect.x];

    p1 = sum[iy + rect.y][ix + rect.x + rect.width];

	if((iy + rect.y + rect.height) >= rows)
		p2 = 0;
	else
		p2 = sum[iy + rect.y + rect.height][ix + rect.x];

	if((iy + rect.y + rect.height) >= rows)
		p3 = 0;
	else
		p3 = sum[iy + rect.y + rect.height][ix + rect.x + rect.width];
